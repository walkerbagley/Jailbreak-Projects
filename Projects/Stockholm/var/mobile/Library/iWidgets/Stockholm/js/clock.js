...
/*
Created by JunesiPhone and edited by Walker Bagley to include seconds.
*/
...
function getFromFormat(format, type, tfh){
    if (type == "time"){
        var elements = format.split(' ').join(':').split(':');
        var newTime = '';
        for (var i = 0; i < elements.length(); i++){
            elements[i] = elements[i].toLowerCase();
            if (elements[i].length <= 2){
                if (elements[i] == 'h' || elements[i] == 'hh'){
                    newTime += clock.hour();
                    if (i + 1 < elements.length() && !(elements[i+1] == 'a')){
                        newTime += ':';
                    }
                }
                else if (elements[i] == 'm' || elements[i] == 'mm'){
                    newTime += clock.minute();
                    if (i + 1 < elements.length() && !(elements[i+1] == 'a')){
                        newTime += ':';
                    }
                }
                else if (elements[i] == 's' || elements[i] == 'ss'){
                    newTime += clock.second();
                }
                else if ((elements[i] == 'a' || elements[i] == 'aa') && !tfh){
                    newTime += ' ' + clock.ampmstrict();
                }
            }
            else if (elements[i].length > 2){
                if (elements[i].substring(0,1) == 'h'){
                    newTime += clock.hourtext();
                    if (i + 1 < elements.length()){
                        newTime += ' ';
                    }
                }
                else if (elements[i] == 'mm'){
                    newTime += clock.minuteonetext() + ' ' + clock.minutetwotext();
                    if (i + 1 < elements.length() && clock.minutetwotext() != ""){
                        newTime += ' ';
                    }
                }
            }
